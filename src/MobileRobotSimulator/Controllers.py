...
class NeuralNetwork(Controller):
    ...
    def getOutput(self, in1, in2):
        activation_n1 = self.activationFunction( in1*0.05 + self.prev_activation_n1*0.5 + self.prev_activation_n2*-0.9 )
        activation_n2 = self.activationFunction( in2*0.05 + self.prev_activation_n2*0.5 + self.prev_activation_n1*-0.9 )
        self.prev_activation_n1 = activation_n1
        self.prev_activation_n2 = activation_n2
        return (activation_n1, activation_n2)
    
    def getAction(self):
        return self.convertWheelSpeedsToTwist( self.getOutput( self.latest_sensor_data[8], self.latest_sensor_data[1] ))
